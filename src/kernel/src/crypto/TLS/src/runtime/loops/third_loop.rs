...
pub struct ThirdLoop {
    channels: Arc<Mutex<BTreeMap<String, Arc<SegQueue<ThirdMessage>>>>>,
...
impl ThirdLoop {
    pub fn new(
        ...
        let sandbox = sandbox_manager.create_sandbox(
            ComponentType::DeviceInterfaces,
    ...
    pub fn sync_sandbox_state(&self) {
        let active = self.session_mgr.get_session(ComponentType::DeviceInterfaces, 0).is_ok()
            || self.session_mgr.get_session(ComponentType::Display, 0).is_ok()
            || self.session_mgr.get_session(ComponentType::Audio, 0).is_ok();

        if active {
            if !self.sandbox.is_active() {
                self.sandbox.activate();
            }
            set_loop_sandbox_active(LoopKind::Third, true);
        } else if self.sandbox.is_active() {
            self.sandbox.deactivate();
            set_loop_sandbox_active(LoopKind::Third, false);
        }
    }
    ...
    fn ensure_loop_flag_active(&self) -> Result<(), &'static str> {
        if is_loop_sandbox_active(LoopKind::Third) {
            Ok(())
        } else {
    ...
    pub fn register_node(&self, node_id: &str, sender: Arc<SegQueue<ThirdMessage>>) {
    ...
    fn validate_io_ui_token(&self, token: &str) -> bool {
        self.session_mgr
            .validate_token_value(ComponentType::DeviceInterfaces, 0, token)
            .unwrap_or(false)
            || self
                .session_mgr
                .validate_token_value(ComponentType::Display, 0, token)
                .unwrap_or(false)
            || self
                .session_mgr
                .validate_token_value(ComponentType::Audio, 0, token)
                .unwrap_or(false)
    }
    ...
    pub(crate) fn receive_external_token(&self, to: &str, token_bytes: Vec<u8>) -> Result<(), &'static str> {
        ...
        if !self.validate_io_ui_token(&token_str) {
            let hp = self.honeypot_system.clone();
            hp.signal_attempt();
    ...
    pub(crate) fn send_message(&self, from: &str, to: &str, payload: Vec<u8>, token: &str) -> Result<(), &'static str> {
        ...
        self.ensure_loop_flag_active()?;
        self.ensure_sandbox_active()?;
        if !self.validate_io_ui_token(token) {
            let hp = self.honeypot_system.clone();
            hp.signal_attempt();
    ...
    pub(crate) fn is_io_or_ui_token(&self, token: &str) -> bool {
        self.validate_io_ui_token(token)
    }
}

#[derive(Clone)]
pub struct ThirdChannel {
    node_id: String,
    third_loop: Arc<ThirdLoop>,
    receiver: Arc<SegQueue<ThirdMessage>>,
}

impl ThirdChannel {
    pub fn new(node_id: String, third_loop: Arc<ThirdLoop>, receiver: Arc<SegQueue<ThirdMessage>>) -> Self {
        Self {
            node_id,
            third_loop,
            receiver,
    ...
    pub fn send(&self, to: &str, payload: Vec<u8>, token: &str) -> bool {
        self.third_loop.send_message(&self.node_id, to, payload, token).is_ok()
    }

    pub fn recv(&self) -> Option<Vec<u8>> {
        if let Some(msg) = self.receiver.pop() {
            self.third_loop.decrypt_message(msg.payload)
        } else {
            None
        }
    }
    ...
    pub fn validate(&self, token: String) -> bool {
        self.third_loop.is_io_or_ui_token(&token)
    }
}
