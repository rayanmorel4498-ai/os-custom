...
fn test_message_parsing() {
    ...
    println!("Message 1 parsed: Intent={:?}, Sentiment={:?}", msg1.intent, msg1.sentiment);
    
    let msg2 = parser.parse("C'est terrible, quelque chose ne fonctionne pas");
    println!("Message 2 parsed: Intent={:?}, Sentiment={:?}", msg2.intent, msg2.sentiment);
    
    let msg3 = parser.parse("Peux-tu aider avec optimisation?");
    println!("Message 3 parsed: Intent={:?}, Sentiment={:?}", msg3.intent, msg3.sentiment);
}
