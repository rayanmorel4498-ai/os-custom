...
fn resource_quota_throttle_realtime_drop_best_effort() {
    let mut quota = ResourceQuotaManager::new();
    quota.set_budget("module:2", 1, 0, 1, 1);
    quota.record_cpu("module:2", 5, 0);
    quota.record_latency("module:2", 5);
    assert!(quota.is_over_budget("module:2"));
    assert_eq!(
        quota.admission_decision("module:2", PriorityClass::Realtime),
        AdmissionDecision::Throttle
    );
    assert_eq!(
        quota.admission_decision("module:2", PriorityClass::BestEffort),
...
fn resource_quota_window_resets_cpu() {
    let mut quota = ResourceQuotaManager::new();
    quota.set_budget("module:3", 2, 0, 10, 100);
