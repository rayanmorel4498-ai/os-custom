    ...
    pub fn simulate_federated_rounds(&self, initial_weights: Vec<f64>) -> Vec<f64> {
        ...
        for client_id in 0..self.num_clients {
            ...
            handles.push(thread::spawn(move || {
                // Simulate local training by perturbing weights
                let mut local_w = weights;
                for _ in 0..local_epochs {
                    for w in local_w.iter_mut() {
                        *w += (rand::random::<f64>() - 0.5) * 0.01; // small local update
                    }
                }
                // Send local model to server
                let _ = tx_clone.send(local_w);
            }));
        }

        drop(tx); // close sending side in main thread

        // Server: collect and average
