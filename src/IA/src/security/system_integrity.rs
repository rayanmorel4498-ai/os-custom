...
/// Rapport d'intégrité d'un composant
...
mod tests {
    ...
    #[tokio::test]
    async fn test_component_check() {
        let monitor = SystemIntegrityMonitor::new();
        assert!(monitor.check_component("kernel", true, None).is_ok());
        let report = monitor.get_component_report("kernel").unwrap();
    ...
    async fn test_system_health() {
        ...
        let _ = monitor.check_component("gpu", false, Some("GPU error")).await;
