...
pub struct SecondaryLoop<S: RuntimeServices> {
...
impl<S: RuntimeServices> SecondaryLoop<S> {
    ...
    fn should_pause_ai(&self, timestamp_ms: u64) -> bool {
        let fallback = ((timestamp_ms / 3_600_000) % 24) as u8;
        let hour = self.services.hour_24(fallback);
        self.services.should_silence(hour)
    }

    fn is_low_power_only(&self) -> bool {
        self.services.energy_pressure() > 0.5
    }

    fn is_over_budget(&self) -> bool {
        self.services.is_over_budget()
    }
    ...
    fn is_over_module_quota(&self, context: &ExecutionContext) -> AdmissionDecision {
        ...
        if usage.is_some() {
            with_quota_api(|quota| quota.admission_decision(&module_key, Self::priority_class(context.priority)))
    ...
    fn is_blocked_by_policy(&self, context: &ExecutionContext, timestamp_ms: u64) -> bool {
        ...
        let decision = with_policy_api(|engine| engine.decide(&key)).unwrap_or(PolicyDecision::Allow);
        match decision {
...
impl Default for SecondaryLoop<GlobalRuntimeServices> {
    fn default() -> Self {
